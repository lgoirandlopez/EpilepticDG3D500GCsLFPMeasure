...
def Network500PPs_1IPfor1window_1PP1GC_PPintervalExploration(IP, nspr, PPintervals, KARfactor, PPnspk,
                                                             folderpathname, cellpath,
                                                             GCstim=1, nGClayer=25, simvar=True, lightstim=False, randseed=False) :
    '''
...
'''
    ...
    if Filenametemp not in os.listdir(folderpathname) :
            OUTPUTPATH=join(folderpathname,Filenametemp)
            os.mkdir(OUTPUTPATH)
    BASEPATH4RESULT = join(folderpathname,Filenametemp)
    ...
    saveopt = False # Option to save result or not
...
'''
    ...
    ******************************************** Set the position of the cells ********************************************
    ...
    secname = 'Soma'
    geompath = 'C:/Users/goirand-lopez/Desktop/Neurones-models/DGview/CoordBySectionCell'
    ...
    *************************************** Set the stimulation trains ***************************************
    ...
    ActivePP = np.random.choice(np.arange(npp),size=(int(GCstim*npp),PPnspk),replace=False)
    ...
    *************************************** Start simulation and record the spikes ***************************************
    ...
    return '--------- '+datetime.datetime.today().strftime("%Y-%m-%d %H:%M")+'\n'+str((IP,nspr,PPintervals,KARfactor,PPnspk,folderpathname,GCstim))+'DONE ---------'
...
if __name__ == '__main__': # protect the entry point
    ...
    tempfolderpathname = 'C:/Users/goirand-lopez/Desktop/Neurones-models/'
                
    model_folder = join('NEURON 7.8 AMD64', 'DG3D500GCsLFPMeasuresTemp')
    cellpath = join(tempfolderpathname,model_folder) 
    
    ParametersSimulation = dict()  
    with open(os.path.join(cellpath, 'ParametersSimulation'+'.txt'), "r") as f:  
        for line in f:
            temp = line.split('\t')
            typval = temp[2]
            key = temp[0]
    ...
    nIP = len(ParametersSimulation['IPlist'])
    sproutingAMPA = ParametersSimulation['nspr']
    PPintervalsAMPA = ParametersSimulation['PPintervals']
    IPlistAMPA = ParametersSimulation['IPlist'] #list
    lightstimAMPA = ParametersSimulation['lightstim']
    randseedAMPA = ParametersSimulation['randseed'] 
    sproutingKA = ParametersSimulation['nspr']
    PPintervalsKA = ParametersSimulation['PPintervals']
    IPlistKA = ParametersSimulation['IPlist']#list
    lightstimKA = ParametersSimulation['lightstim']
    randseedKA = ParametersSimulation['randseed']
    
    
    TASKS += [(IP,ParametersSimulation['nspr'],ParametersSimulation['PPintervals'],0,ParametersSimulation['PPnspk'],folderpathname,cellpath,ParametersSimulation['GCstim'],ParametersSimulation['nGClayer'],
                ParametersSimulation['simvar'],ParametersSimulation['lightstim'],ParametersSimulation['randseed']) for IP in IPlistAMPA]
    TASKS += [(IP,ParametersSimulation['nspr'],ParametersSimulation['PPintervals'],.5,ParametersSimulation['PPnspk'],folderpathname,cellpath,ParametersSimulation['GCstim'],ParametersSimulation['nGClayer'],
               ParametersSimulation['simvar'],ParametersSimulation['lightstim'],ParametersSimulation['randseed']) for IP in IPlistKA]
    
    # IP = 1
    # Network500PPs_1IPfor1window_1PP1GC_PPintervalExploration(IP,ParametersSimulation['nspr'],ParametersSimulation['PPintervals'],0,ParametersSimulation['PPnspk'],folderpathname,cellpath,ParametersSimulation['GCstim'],ParametersSimulation['nGClayer'],
